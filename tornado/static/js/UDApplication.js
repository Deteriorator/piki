...
Application.prototype._selectDiagram = function(b) {
	if (this._selected && this._diagrams[b].isVisible()) {
		this._diagrams[this._selected].interaction(false);/* http://www.jrromero.net/tools/jsUML2 */
...
Application.prototype._generateGeneralMenu = function() {
	...
	b.setAttribute("src", "/static/img/uml/vertical_flow.png");
	c.appendChild(b);
	c.appendChild(f);
	...
	c.onclick = function() {
		...
		if (j.style.display == "none") {
			...
			this.childNodes[0].setAttribute("src",
					"/static/img/uml/vertical_flow.png")
		} else {
			j.style.display = "none";
			this.childNodes[0].setAttribute("src",
					"/static/img/uml/horizontal_flow.png")
		}
	};
	this._addMenuItem(this._tools_ul1, "Export/Import to xml", function(m) {
		...
		c.addEventListener("click", o, false);
	...
	this._addMenuItem(this._tools_ul1, "Generate image (png)", function(k, j) {
		if (k._selected) {
			var h = window.open(k._getDiagramImage(), "diagramImage", "height="
					+ (k._height + 70) + ",width=" + (k._width + 20));
			h.focus()
		} else {
	...
	this._addMenuItem(this._tools_ul1, "Delete object", function(l, h, p) {
		...
		if (n) {
			...
			ApplicationValues.dialog._text = "Do you want to delete the object"
					+ m + " (" + n.getType() + ") ?";
			ApplicationValues.dialog._cancelable = true
		}
	...
	this._addMenuItem(this._tools_ul1, "Modify color", function(x) {
		...
		var C = new Array(parseInt(z.slice(0, 2), 16), parseInt(z.slice(2, 4),
				16), parseInt(z.slice(4, 6), 16));/* http://www.jrromero.net/tools/jsUML2 */
		...
		B.height = 20;
		s.appendChild(B);
		var t = B.getContext("2d");/* http://www.jrromero.net/tools/jsUML2 */
		...
		var E = function(N, M, O, L) {
			...
			M.fillStyle = "#000000";
			M.beginPath();
			M.arc(20 + (O * 100) / 255, parseInt(N.height) / 2, 4, 0,
					Math.PI * 2, true);
			M.closePath();
			M.fill();
			M.save();
		...
		s.addEventListener("mousedown", k, false);/* http://www.jrromero.net/tools/jsUML2 */
		v.addEventListener("mousedown", k, false);
		document.body.appendChild(A);
		A.style.top = (window.innerHeight - parseInt(A.offsetHeight)) / 2
				+ "px";
	...
	if (this._selected) {
		...
		for (c in a) {
			if (a[c].length == 4) {
				...
				if (a[c].length == 3) {
					this._addMenuItem(this._tools_ul2, a[c][0], a[c][1],
							a[c][2])
				}
			}
		}
...
Application.prototype._addMenuItem = function(a, g, f, k, b) {
	...
	if (b) {
		...
		c.setAttribute("src", "/static/img/uml/" + b + ".png");
		h.appendChild(c)
	}
	e.appendChild(j);
	h.appendChild(e);
...
Application.prototype.getXML = function() {
	...
	var a;
	var c;
	for (c in this._diagrams) {/* http://www.jrromero.net/tools/jsUML2 */
...
Application.prototype.setXML = function(xml) {
	...
	var xmlnodes = application.childNodes;/* http://www.jrromero.net/tools/jsUML2 */
	var aux, nodeName;
	var i, j;
...
_acceptedDiagrams.push("UMLUseCaseDiagram");/* jsUML2.UCO.es */
...
_specificMenu.UMLUseCaseDiagram = [ [ "Actor", function(b, a, d) {
	...
	if (b && b.getType() == "UMLGeneralizationSet") {
		...
		if (a && a.getType() == "UMLGeneralizationSet") {/* http://www.jrromero.net/tools/jsUML2 */
			a.addElement(b);
			e.addElement(a.getRelation(b))
		} else {
			e.addRelationFromPoints(new UMLGeneralization(), d, g, c, f)
...
_specificMenu.UMLComponentDiagram = [
		[ "Package", function(b, a, d) {/* http://www.jrromero.net/tools/jsUML2 */
			var c = new UMLPackage({
				x : a,
				y : d,
				stereotypes : _stereotypes
		...
		}, 2, "UMLConnector" ], ];/* http://www.jrromero.net/tools/jsUML2 */
...
}, 0, "UMLStateMachineDiagram" ]);/* http://www.jrromero.net/tools/jsUML2 */
		...
		[
				...
				function(c, b, f, a, e) {
					...
					if (j instanceof NodeForkJoin) {
						g = (j._quadrant == 1) ? (b - j._x) : 0;/* http://www.jrromero.net/tools/jsUML2 */
...
Application.prototype.setProfileSpecificMenu = function() {
	var a = this;
	_specificMenu.UMLProfile = [ [ "Metaclass", function(d, b, e) {
		...
		_metaclass.push(c);
		d.addElement(c)
	}, 1, "UMLMetaclass" ], [ "Stereotype", function(c, b, e) {
		...
		_stereotypes.push(d);
		c.addElement(d)
	}, 1, "UMLStereotype" ], [ "Extension", function(d, c, f, b, e) {
