...
AcceptEventAction.prototype.getLinkCentered = function(n, l) {
	...
	if (n < d) {
		...
		if (l < c) {
			...
			var b;
			b = (this.getY() + this.getHeight() - c)
					/ (this.getX() + this.getWidth() - d);
			if (((l - c) == 0) || ((l - c) / (n - d)) < b) {
				a = this.getX() + this.getWidth();
				g = this.getX() + this.getWidth();
				e = this.getY() + this.getHeight()
	...
	if (this._components.length > 0) {
		...
		for (b in this._components) {
			...
			if (a.getPosition() == Component.Float
					|| a.getPosition() == Component.BottomLeft
					|| a.getPosition() == Component.BottomRight) {
				c += a.getHeight();
				if (a.getWidth() > d) {
					d = a.getWidth()
				}
...
AcceptEventAction.prototype.updateComponents = function() {
	if (this._components.length > 0) {
		this.calculateSize();
		this.insertComponents(this._x + 25, this._y, this._width - 25,
				this._height);
		var a;
...
ActivityDiagram.prototype._instantiateElements = function(d, c, b) {
	...
	if (e) {
		...
		for (a = 0; a < d.childNodes.length; a++) {
			if (e instanceof SuperNode
					&& d.childNodes[a].nodeName == "Swimlane") {
...
ActivityDiagram.prototype._instantiateObjectFromString = function(elemName,
		parent) {
	if (JSFun.isString(elemName)) {
		...
		for (i in this._validElements) {
			if (elemName == this._validElements[i]) {
				if (elemName == "UMLHorizontalHierarchicalSwimlane"
						|| elemName == "UMLVerticalHierarchicalSwimlane"
						|| elemName == "UMLHorizontalSwimlane"
						|| elemName == "UMLVerticalSwimlane"
						|| elemName == "UMLFlow") {
					var setElementXml = true;
					return eval("new " + this._validElements[i]
							+ "({ setElementXml: " + setElementXml + "})")
				} else {
					if (parent) {
...
ExceptionHandler.prototype.select = function(a, c) {
	...
	for (b = 0; b < this._points.length; b++) {
		if (Math.abs(a - this._points[b].getX()) <= 4
				&& Math.abs(c - this._points[b].getY()) <= 4) {
			if (this._selected > -1) {
	...
	for (var b = 0; b < this._points.length - 1; b++) {
		if (this._selectLine(this._points[b], this._points[b + 1], a, c)) {
			...
			this._points.splice(this._selected + 1, 0, new Point(a, c));
			return true
		}
	}
	return false
...
ExceptionHandler.prototype.draw = function(b) {
	...
	if (l % 2 != 0) {
		...
		var h = (a + o) / 2;
		var e = (this._components.length) ? this._components[0]._y - 30
				: ((p + m) / 2 - 30)
	}
	...
	if (o < g) {
		if (m < d) {
			if (Math.abs(o - g) > Math.abs(m - d)) {
				...
				var h = (this._components.length) ? this._components[0]._x + 30
						: ((a + o) / 2 + 30);
				var e = (p + m) / 2
			}
		} else {
...
Pin.prototype.drop = function(a, b) {
	this.correctPosition();/* http://www.jrromero.net/tools/jsUML2 */
...
Pin.prototype.correctPosition = function() {
	...
	if (this.getHeight() > this.getWidth()) {
		var g = this._action.getLinkCentered(this.getX() + this._width, this
				.getY()
				+ this._height / 2)
	} else {
		var g = this._action.getLinkCentered(this.getX() + this._width / 2,
				this.getY() + this._height)
	}
	var b = g.getX();
	var l = g.getY();
	if (this.getHeight() > this.getWidth()) {
		if ((this._action.getY() == l)
				|| ((this._action.getY() + this._action.getHeight()) == l)) {
		...
		if (this.getWidth() > this.getHeight()) {
			if ((this._action.getX() == b)
					|| ((this._action.getX() + this._action.getWidth()) == b)) {
				var a = this.getHeight();
				this.setHeight(this.getWidth());
				this.setWidth(a)
			}
...
Pin.prototype.calculateSize = function() {
	if (this._components.length > 0) {
		...
		for (b in this._components) {
			a = this._components[b];
			if (a._visible
					&& !(a instanceof RegionLine)
					&& (a.getPosition() == Component.Float
							|| (a.getPosition() == Component.BottomLeft && a._visible)
							|| a.getPosition() == Component.BottomRight || a
							.getPosition() == Component.Xmovement)) {
				if (a._orientation) {
					d += a.getWidth();
					if (a.getHeight() > c) {
						c = a.getHeight()
					...
					c += a.getHeight();
					g = (a.getPosition() == Component.Xmovement) ? (a
							.getWidth() + 2 * a._parent._Xmovement) : a
							.getWidth();
					if (g > d) {
						d = g
					}
...
Flow.prototype.setElementXML = function(h, e) {
	...
	for (d = 0; d < g.length; d++) {
		if (g[d].nodeName == "point") {
			this._points[l] = new Point(parseInt(g[d].getAttribute("x")),
					parseInt(g[d].getAttribute("y")));
			l++
		}
	}
	this._addComponents();
...
Flow.prototype.updateName = function() {
	...
	if (this._elemA.getType() != "UMLDecision_MergeNode"
			&& this._name instanceof GuardItem) {
...
Flow.prototype._calculateLineEnds = function() {
	if (this._elemA.getType() == "UMLFork_JoinNode"
			|| this._elemB.getType() == "UMLFork_JoinNode") {
		...
		if (!this._selectedPoint && this._elemA && this._elemB) {
			if (d > 2) {
				if (this._elemA.getType() == "UMLFork_JoinNode") {
					if (this._x != -1) {
						this._points[0].setPoint(this._elemA.getLink(
								this._points[1]._x, this._points[1]._y,
								this._elemA._x + this._x, b._y))
					} else {
						this._points[0].setPoint(this._elemA.getLink(
								this._points[1]._x, this._points[1]._y, b._x,
								this._elemA._y + this._y))
					}
					this._elemA.updateLimitSize()
				} else {
					this._points[0] = this._elemA
							.getLinkCentered(this._points[1])
				}
				if (this._elemB.getType() == "UMLFork_JoinNode") {
					if (this._x != -1) {
						this._points[d - 1].setPoint(this._elemB.getLink(
								this._points[d - 2]._x, this._points[d - 2]._y,
								this._elemB._x + this._x, a._y))
					} else {
						this._points[d - 1].setPoint(this._elemB.getLink(
								this._points[d - 2]._x, this._points[d - 2]._y,
								a._x, this._elemB._y + this._y))
					}
					this._elemB.updateLimitSize()
				} else {
					this._points[d - 1] = this._elemB
							.getLinkCentered(this._points[d - 2])
				}
			} else {
				if (this._elemA.getType() == "UMLFork_JoinNode") {
					if (this._x != -1) {
						this._points[0].setPoint(this._elemA.getLink(a._x,
								a._y, this._elemA._x + this._x, b._y))
					} else {
						this._points[0].setPoint(this._elemA.getLink(a._x,
								a._y, b._x, this._elemA._y + this._y))
					}
					this._elemA.updateLimitSize()
				} else {
					c = true
				...
				if (this._elemB.getType() == "UMLFork_JoinNode") {
					if (this._x != -1) {
						this._points[1].setPoint(this._elemB.getLink(b._x,
								b._y, this._elemB._x + this._x, a._y))
					} else {
						this._points[1].setPoint(this._elemB.getLink(b._x,
								b._y, a._x, this._elemB._y + this._y))
					}
					this._elemB.updateLimitSize()
				} else {
					this._points[1] = this._elemB.getLinkCentered(
							this._points[0].getX(), this._points[0].getY())
				}
				if (c) {
					this._points[0] = this._elemA.getLinkCentered(
							this._points[1].getX(), this._points[1].getY())
				}
			}
		}
	} else {
...
Flow.prototype._checkForNewNodes = function(a, e) {
	if (this._selectedPoint
			&& (this._selected == 0 || this._selected == this._points.length - 1)) {
		var b = this._diagram.reassignRelationTo(this, a, e);
		if (b != this) {
			if (this._selected == 0) {
				if (b
						&& !(b.getType() == "UMLFork_JoinNode" && this._elemB
								.getType() == "UMLFork_JoinNode")) {
					var d = this._elemA;
					this.setElementA(b);
				...
				if (b
						&& !(b.getType() == "UMLFork_JoinNode" && this._elemA
								.getType() == "UMLFork_JoinNode")) {
					var c = this._elemB;
					this.setElementB(b);
...
HierarchicalSwimlane.prototype.resetSizeComponentSwimlane = function() {
	...
	for (var b = 0; b < a.length; b++) {
		a[b]._heightComp = (this._orientation) ? (a[b]._components[0]._height + a[b]._components[1]._height)
				: 0;
		a[b]._widthComp = (this._orientation) ? 0
				: (a[b]._components[0]._width + a[b]._components[1]._width)
	}
};
...
HierarchicalSwimlane.prototype.updateRegions = function(b, l) {
	...
	if (this._orientation) {
		...
		for (var c = 0; c < e; c++) {
			...
			o.setMinHeight(this._minHeight - this._components[0].getHeight()
					- this._components[1].getHeight());
			o.setHeight(this.getHeight() - this._components[0].getHeight()
					- this._components[1].getHeight());
			if (o._components[2] instanceof RegionLine) {
				o._components[2].setHeight(this.getHeight()
						- this._components[0].getHeight()
						- this._components[1].getHeight())
			}
		}
		for (c = 0; c < e; c++) {
			...
			if (c == e - 1) {
				if (b
						|| (h + this._nodeChilds[c]._width) < (this.getWidth() + this
								.getX())) {
					this._nodeChilds[c].setWidth(this.getWidth() + this.getX()
							- h)
				} else {
		...
		for (c = 0; c < e; c++) {
			...
			if (n > 0 || (n < 0 && !l)) {
				for (var d = 0; d < this._nodeChilds[c]._nodeChilds.length; d++) {
					this._nodeChilds[c]._nodeChilds[d].updatePosition(n, 0,
							true)
				}
			}
		...
		for (var c = 0; c < e; c++) {
			var o = this._nodeChilds[c];
			o.setMinWidth(this._minWidth - this._components[0].getWidth()
					- this._components[1].getWidth());
			o.setWidth(this.getWidth() - this._components[0].getWidth()
					- this._components[1].getWidth());
			if (o._components[2] instanceof RegionLine) {
				o._components[2].setWidth(this.getWidth()
						- this._components[0].getWidth()
						- this._components[1].getWidth())
			}
		}
		for (c = 0; c < e; c++) {
			...
			if (c == e - 1) {
				if (b
						|| (g + this._nodeChilds[c]._height) < (this
								.getHeight() + this.getY())) {
					this._nodeChilds[c].setHeight(this.getHeight()
							+ this.getY() - g)
				} else {
					this.setHeight(g + this._nodeChilds[c]._height
							- this.getY())
				}
			}
		...
		for (c = 0; c < e; c++) {
			...
			if (n > 0 || (n < 0 && !l)) {
				for (var d = 0; d < this._nodeChilds[c]._nodeChilds.length; d++) {
					this._nodeChilds[c]._nodeChilds[d].updatePosition(0, n,
							true)
				}
			}
			this._nodeChilds[c].resetMovement()
...
HierarchicalSwimlane.prototype.select = function(l, h) {
	...
	if (this._diagram._pressMouse == true) {
		if (this._selected) {
			if (this._moveable
					&& Math.abs(l - (this._x + this._width + 2.5)) <= 5
					&& Math.abs(h - (this._y + this._height + 2.5)) <= 5) {
				this._resizing = true;
				return true
			}
			var a = this._nodeChilds;
...
HierarchicalSwimlane.prototype.updateContainer = function(l) {
	...
	if (this._container) {
		var p;
		var c = (this._orientation) ? (this._components[0]._height + this._components[1]._height)
				: 0;
		var g = (this._orientation) ? 0
				: (this._components[0]._width + this._components[1]._width);
		var m = this._x + g;
		var h = this._y + c;
		var d = this._x + g;
		var b = this._y + c;
...
var UMLStereotypedElement = function(a) {
	...
	this._parent = a.parent || null;/* http://www.jrromero.net/tools/jsUML2 */
	this._shownStereotype;
	this._appliedStereotypes = []
};
UMLStereotypedElement.prototype.isDrawableStereotype = function() {
	var b = [ "UMLActor", "UMLUseCase", "UMLUseCaseExtended", "UMLSystem",
			"UMLSubSystem", "UMLClass", "UMLComponent", "UMLInterfaceExtended",
			"UMLPackage", "UMLPackageContainer", "UMLComComponent",
			"UMLInterface", "UMLOption", "UMLAlternative", "UMLLoop",
			"UMLBreak", "UMLAcceptEventAction", "UMLTimeEvent",
			"UMLSendSignalAction", "UMLAction", "UMLObject", "UMLActivity",
			"UMLDataStore", "UMLConnectorActivity",
			"UMLHorizontalHierarchicalSwimlane",
			"UMLVerticalHierarchicalSwimlane", "UMLSimpleState",
			"UMLCompositeState", "UMLVerticalRegion", "UMLPin",
			"UMLParameterNode", "UMLExpansionNode", "UMLHorizontalRegion",
			"UMLPort", "UMLTerminate", "UMLEntryPoint", "UMLExitPoint",
			"UMLJunction", "UMLFlowFinal", "UMLDataType" ];
...
UMLStereotypedElement.prototype.applyStereotype = function(b) {
	...
	for (c = 0; c < d.getStereotype()._childs.length && !e; c++) {
		if (a == d.getStereotype()._childs[c]._text
				&& b._validMetaclass(d.getType())) {
			e = true
		}
	}
	if (!e) {
...
UMLStereotypedElement.prototype.removeFigure = function(a) {
	...
	for (b = 0; b < this._parent._figures.length; b++) {
		if (this._parent._figures[b] instanceof FromImageFigure
				&& this._parent._figures[b].foundInAssociatedStereotypes(a)) {
			...
			for (var b = 0; b < a.length && !e; b++) {
				if ("\xAB" + this._appliedStereotypes[c].getName() + "\xBB" == a[b]._text) {
					for (k = 0; k < this._appliedStereotypes[c]._components[3]._childs.length; k++) {
						d
								.push([
										this._appliedStereotypes[c]._components[3]._childs[k]
												.getNameTagValue(),
										this._appliedStereotypes[c]._components[3]._childs[k]
												.getDefaultValue() ])
					}
				...
				for (m = 0; m < r._stereotypeProperties._stereotypes.length; m++) {
					...
					for (l = 0; l < t.length; l++) {
						if (t[l].getName() == r.getType()) {
							if (!n["\xAB"
									+ o._stereotypeProperties._stereotypes[m]
											.getName() + "\xBB"]) {
								n["\xAB"
										+ o._stereotypeProperties._stereotypes[m]
												.getName() + "\xBB"] = o._nodeChilds[h]
							}
						}
					}
				}
...
UMLStereotypedElement.prototype.showStereotypesDialog = function() {
	...
	for (m in n) {
		...
		e.setAttribute("value", m);
		e.appendChild(document.createTextNode(m));
		if (o._stereotypeProperties._shownStereotype
				&& o._stereotypeProperties._shownStereotype == m) {
...
StereotypeTag.prototype.setText = function(d, b) {
	...
	if (JSFun.isString(d)) {
		if (this._text != d && !b) {
			if (this._parent && this._parent instanceof SuperComponent
					&& this._parent._parent._stereotypeProperties) {
				...
				for (c = 0; c < a._appliedStereotypes.length; c++) {
					if ("\xAB" + a._appliedStereotypes[c].getName() + "\xBB" == this._text) {
						this._parent._parent._stereotypeProperties
								.removeStereotype(a._appliedStereotypes[c]);
						break
					}
				}
			} else {
				if (this._parent && this._parent._stereotypeProperties) {
					...
					for (c = 0; c < a._appliedStereotypes.length; c++) {
						if ("\xAB" + a._appliedStereotypes[c].getName()
								+ "\xBB" == this._text) {
...
NodeForkJoin.prototype.setElementXML = function(d) {
	this.setPosition(parseInt(d.getAttribute("x")), parseInt(d
			.getAttribute("y")));
	this.resetMovement();
...
ParameterNode.prototype.correctPosition = function() {
	...
	var c = this._parent.getWidth();
	var d = this._parent.getHeight();
	var e = this._parent.getLinkCentered(this.getX() + this._width / 2, this
			.getY()
			+ this._height / 2);
	var a = e.getX();
	var g = e.getY();
...
SwimlaneLine.prototype.drawShape = function(d) {
	...
	if (this._parent.getParent()._orientation) {
		d.strokeRect(this.getPixelX() - 2, this.getPixelY(),
				this.getWidth() + 4, this.getHeight())
	} else {
		d.strokeRect(this.getPixelX(), this.getPixelY() - 4, this.getWidth(),
				this.getHeight() + 6)
	}
	d.restore();
	d.save();
	...
	d.beginPath();
	if (this._parent.getParent()._orientation) {
		d.arc(this._parent.getX() + this._parent.getWidth() - 10, this._parent
				.getY()
				+ this._parent._heightComp + 7, 4, 0, Math.PI * 2, true)
	} else {
		d.arc(this._parent.getX() + this._parent._widthComp + 7, this._parent
				.getY()
				+ this._parent.getHeight() - 10, 4, 0, Math.PI * 2, true)
	}
	d.closePath();
	d.fill();
	d.restore();
	...
	d.beginPath();
	if (this._parent.getParent()._orientation) {
		d.arc(a[c + 1].getX() + a[c + 1].getWidth() - 10, a[c + 1].getY()
				+ a[c + 1]._heightComp + 7, 4, 0, Math.PI * 2, true)
	} else {
		d.arc(a[c + 1].getX() + a[c + 1]._widthComp + 7, a[c + 1].getY()
				+ a[c + 1].getHeight() - 10, 4, 0, Math.PI * 2, true)
	}
	d.closePath();
	d.fill();
	d.restore()
...
Swimlane.prototype.addComponents = function(g) {
	...
	if (this._parent._orientation) {
		if (b > 0 && g) {
			...
			this.setWidth(this._parent.getX() + this._parent.getWidth()
					- this._x)
		}
		...
		if (b > 0) {
			...
			if (this._parent instanceof HierarchicalSwimlane) {
				var a = this._parent.getHeight()
						- this._parent._components[0]._height
						- this._parent._components[1]._height
			} else {
				var a = this._parent.getHeight()
			}
		...
		if (b > 0 && g) {
			...
			this.setHeight(this._parent.getY() + this._parent.getHeight()
					- this._y)
		}
		this.addComponent(new StereotypeFields({
			id : "stereotypes",
		...
		if (b > 0) {
			...
			if (this._parent instanceof HierarchicalSwimlane) {
				var e = this._parent.getWidth()
						- this._parent._components[0]._width
						- this._parent._components[1]._width
			} else {
				var e = this._parent.getWidth()
			}
			d[b - 1].addComponent(new SwimlaneLine({
...
Swimlane.prototype.notifyChange = function() {
	if (this._parent._orientation) {
		this._heightComp = this._components[0]._height
				+ this._components[1]._height;
		this._widthComp = 0
	} else {
		this._heightComp = 0;
		this._widthComp = this._components[0]._width
				+ this._components[1]._width
	}
...
SwimlaneItem.prototype.createRegion = function() {
	...
	if (this.getParent()._orientation) {
		if (this._parent instanceof HierarchicalSwimlane) {
			var a = this.getParent()._components[0]._height
					+ this.getParent()._components[1]._height
		} else {
			var a = 0
		}
		...
		if (this._parent instanceof HierarchicalSwimlane) {
			var c = this.getParent()._components[0]._width
					+ this.getParent()._components[1]._width
		} else {
			var c = 0
		}
		this.getParent().addRegion(new Swimlane({
...
UnhierarchicalSwimlane.prototype.select = function(m, l) {
	...
	if (this._diagram._pressMouse == true) {
		if (this._selected) {
			if (this._moveable
					&& Math.abs(m - (this._x + this._width + 2.5)) <= 5
					&& Math.abs(l - (this._y + this._height + 2.5)) <= 5) {
				this._resizing = true;
				return true
			}
			var a = this._nodeChilds;
			for (c = 0; c < a.length - 1; c++) {
				if (c == h) {
					if (this._orientation) {
						...
						var b = a[c]._components[0]._width
								+ a[c]._components[1]._width;
...
var UMLActivityDiagram = function(b) {
	...
	a.setValidElements([ "UMLAcceptEventAction", "UMLTimeEvent", "UMLAction",
			...
			"UMLConnectorActivity", "UMLHorizontalSwimlane",
			"UMLVerticalSwimlane", "UMLVerticalSwimlane",
			"UMLHorizontalHierarchicalSwimlane",
			"UMLVerticalHierarchicalSwimlane", "UMLExceptionHandler", "Region",
			"Swimlane", "UMLPin", "UMLParameterNode", "UMLExpansionNode",
			"UMLNote", "UMLLine" ]);
	return a
};
...
var UMLClassDiagram = function(b) {
	...
	a.setValidElements([ "UMLNote", "UMLLine", "UMLClass", "UMLDataType",
			"UMLComponent", "UMLInterfaceExtended", "UMLPackage",
			"UMLPackageContainer", "UMLAggregation", "UMLAssociation",
			"UMLComposition", "UMLDependency", "UMLGeneralization",
			"UMLRealization", "UMLUsage", "UMLPackageMerge",
			"UMLPackagePublicImport", "UMLPackagePrivateImport",
			"UMLGeneralizationSet", "SetLine", "UMLNAssociation",
			"AssociationN" ]);
	return a
};
...
ComponentDiagram.prototype.addRelationFromPoints = function(c, b, l, a, g) {
	...
	var e;
	if (p && o) {
		if (c.getType() == "UMLGeneralization" && p instanceof Node
				&& o instanceof Node) {
			...
			for (e = 0; e < p._relations.length && !n; e++) {
				if (p._relations[e].getType() == "UMLGeneralization"
						&& p._relations[e]._elemA == p) {
					var d = new UMLGeneralizationSet({
						x : p._relations[e].getCentralPoint().getX(),
						y : p._relations[e].getCentralPoint().getY()
					});
					d.setSuperClass(p);
					this.addElement(d);
					var q = new UMLSetLine();
					...
					if (p._relations[e].getType() == "UMLSetLine"
							&& p._relations[e]._elemA == p) {
						var q = new UMLSetLine();
...
NodePorts.prototype.addChild = function(a) {
	if (a instanceof Port) {
		this._ports.push(a);
		a.setParent(this);
		a.correctPosition();/* http://www.jrromero.net/tools/jsUML2 */
		return
	} else {
...
var UMLComponentDiagram = function(b) {
	...
	a.setValidElements([ "UMLNote", "UMLLine", "UMLComComponent",
			"UMLComposition", "UMLInterface", "UMLInterfaceExtended",
			"UMLInterfaceUsage", "UMLInterfaceRealization", "UMLPort",
			"UMLGeneralization", "UMLRealization", "UMLInterfaceExtended",
			"UMLConnector", "UMLPackage", "UMLPackageContainer",
			"UMLGeneralizationSet", "SetLine" ]);
	return a
};
var UMLPackage = function(b) {
	var b = b || {};
...
Line.prototype.draw = function(b) {
	var c = this._points.length;
	if (this._line) {
		this._line.draw(b, this._points, this.getLineColor(), this
				.getLineWidth())
	}
	if (this._selected >= 0) {
		var a;
		for (a = 0; a < this._points.length; a++) {
			b.fillRect(parseInt(this._points[a].getX()) - 3,
					parseInt(this._points[a].pixelY()) - 3, 6, 6)
		}
	}
	if (this._selected > -1) {
...
Metaclass.prototype.setValidMetaclass = function(b) {
	...
	this._validMetaclassLibrary = [ "UMLActor", "UMLUseCase",
			"UMLUseCaseExtended", "UMLSystem", "UMLSubSystem", "UMLLine",
			"UMLClass", "UMLComponent", "UMLInterfaceExtended", "UMLPackage",
			"UMLPackageContainer", "UMLComComponent", "UMLInterface",
			"UMLLifeline", "UMLOption", "UMLAlternative", "UMLLoop",
			"UMLBreak", "UMLAcceptEventAction", "UMLTimeEvent",
			"UMLSendSignalAction", "UMLAction", "UMLObject", "UMLActivity",
			"UMLDataStore", "UMLConnectorActivity",
			"UMLHorizontalHierarchicalSwimlane",
				...
				for (var c = 0; c < b.length; c++) {
					if (b[c]._stereotypeProperties && this._parent) {
						for (var a = 0; a < this._parent._stereotypes.length; a++) {
							b[c]._stereotypeProperties
									.removeStereotype(this._parent._stereotypes[a])
						}
					}
				}
			}
...
MetaclassItem.prototype.showDialog = function() {
	...
	for (var b = 0; b < this._parent._validMetaclassApp.length; b++) {
		...
		g.appendChild(document
				.createTextNode(this._parent._validMetaclassApp[b][0]));
				...
				for (var c = 0; c < b.length; c++) {
					if (b[c]._stereotypeProperties && this._parent) {
						if (this._parent._validMetaclass(b[c].getType())) {
							b[c]._stereotypeProperties
									.removeFigure(this._parent);
							if (b[c]._stereotypeProperties._shownStereotype == "\xAB"
									+ this._parent.getName() + "\xBB") {
								b[c]._stereotypeProperties
										.drawStereotype(this._parent)
							}
						}
					}
				}
...
SrcItem.prototype.draw = function(a) {
	...
	if (this.active) {
		a.fillStyle = "#ffc485";
		a.fillRect(this.getPixelX(), this.getPixelY(), this.getWidth(), this
				.getHeight())
	}
	a.restore();
	a.save();
	a.font = this.getFontStyle() + " " + this.getFontWeight() + " "
...
Stereotype.prototype.delMetaclass = function(c) {
	...
	for (b in this._diagrams) {
		for (var a = 0; a < this._diagrams[b]._nodes.length; a++) {
			if (this._diagrams[b]._nodes[a].getType() == c.getName()
					&& this._diagrams[b]._nodes[a]._stereotypeProperties) {
				this._diagrams[b]._nodes[a]._stereotypeProperties
						.removeStereotype(this)
			}
		}
	}
				...
				for (var b = 0; b < a.length; b++) {
					if (a[b]._stereotypeProperties && this._parent) {
						if (this._parent._validMetaclass(a[b].getType())) {
							a[b]._stereotypeProperties.changeNameStereotype(
									this._parent, "\xAB" + d + "\xBB")
						}
					}
				}
			}
...
TagValueItem.prototype.select = function(a, b) {
	if (Math.abs(a - (this.getPixelX() + this.getSuperWidth() - 20)) <= 5
			&& Math.abs(b - (this.getPixelY() + 8.66)) <= 5) {
		this.notifyToUp();
		this.notifyChange();
		return true
	} else {
		if (Math.abs(a - (this.getPixelX() + this.getSuperWidth() - 30)) <= 5
				&& Math.abs(b - (this.getPixelY() + 7.33)) <= 5) {
			this.notifyToDown();
			this.notifyChange();
			return true
		}
...
var UMLProfile = function(b) {
	...
	a.setName("Profile");
	a.setValidElements([ "UMLNote", "UMLLine", "UMLMetaclass", "UMLExtension",
			"UMLStereotype" ]);
	return a
};
...
Alternative.prototype.drop = function(m, l) {
	...
	if (this._moved) {
		...
		for (d = 0; d < n.length; d++) {
			if (this.isOverBeforePosition(n[d].getCentralPoint())
					&& n[d] instanceof Message) {
...
SequenceDiagram.prototype.checkForParent = function(a) {
	if ((a instanceof Node) && !(a instanceof Lifeline)
			&& !(a instanceof TimeInterval)) {
...
SequenceDiagram.prototype._sortNodesByArea = function() {
	this._nodes
			.sort(function(e, c) {
				var g = e.getArea();
				var d = c.getArea();
				if ((e.getType() == "UMLLifeline" && c.getType() != "TimeInterval")
						|| (e.getType() != "TimeInterval" && c.getType() == "UMLLifeline")) {
...
SequenceDiagram.prototype._instantiateObjectFromString = function(elemName,
		parent) {
	if (JSFun.isString(elemName)) {
		parent = parent || null;
		var i;
		for (i in this._validElements) {
			if (elemName == this._validElements[i]) {
				if (elemName == "UMLAlternative" || elemName == "UMLCreate"
						|| elemName == "UMLDestroy"
						|| elemName == "UMLSendMessage"
						|| elemName == "UMLCallMessage"
						|| elemName == "UMLDeleteMessage"
						|| elemName == "UMLReplyMessage") {
					var setElementXml = true;
					return eval("new " + this._validElements[i]
							+ "({ setElementXml: " + setElementXml + "})")
				} else {
...
Message.prototype._calculateLineEnds = function(n) {
	...
	if (!this._selectedPoint && this._elemA && this._elemB) {
		...
		if (n) {
			...
			if (this._objA
					|| (this._elemA instanceof TimeInterval && this._objB)) {
				var e = [];
				...
				for (d = 0; d < e.length; d++) {
					if (!e[d]._moved) {
						if (this._objA) {
							...
							a = (this._objB) ? (this._objB._y - this._objB._prey)
									: 0
						}
						e[d]._y = e[d]._y + a;
						e[d]._moved = true;
			...
			if (this._objB
					|| (this._elemB instanceof TimeInterval && this._objA)) {
				var b = [];
				...
				for (d = 0; d < b.length; d++) {
					if (!b[d]._moved) {
						if (this._objB) {
							a = (this._objB) ? (this._objB._y - this._objB._prey)
									: (this._objA._y - this._objA._prey)
						} else {
							a = (this._objA) ? (this._objA._y - this._objA._prey)
									: 0
						}
						b[d]._y = b[d]._y + a;
						b[d]._moved = true;
	...
	a
			.sort(function(d, c) {
				if (d instanceof CreateMessage && c instanceof CreateMessage) {
					...
					if (d instanceof CreateMessage
							&& !(c instanceof CreateMessage)) {
						return b
					} else {
						if (!(d instanceof CreateMessage)
								&& c instanceof CreateMessage) {
...
Message.prototype.updateLimitY = function() {
	...
	var a = 0;
	if (this._elemA) {
		b = (this._elemA instanceof Lifeline) ? (this._elemA.getY()
				+ this._elemA._heightSmallRectangle + 5)
				: (this._elemA.getY() + 5)
	}
	if (this._elemB) {
		a = (this._elemB instanceof Lifeline) ? (this._elemB.getY()
				+ this._elemB._heightSmallRectangle + 5)
				: (this._elemB.getY() + 5)
	}
...
Message.prototype.notifyChange = function() {
	...
	for (b = 0; b < a.length; b++) {
		if (((a[b].getType() == "UMLLifeline") && !(a[b]._delete))
				|| ((a[b].getType() == "UMLLifeline") && (a[b]._delete) && ((a[b] == this._elemA) || (a[b] == this._elemB)))
				|| (a[b].getType() == "TimeInterval")) {
			a[b].updateLength()
		}
	}
};
...
Message.prototype.updateObjects = function() {
	if (this._objA) {
		this._objA.setPosition(this._points[0].getX() - this._objA.getWidth()
				/ 2, this._points[0].getY())
	}
	if (this._objB) {
		if (this._elemA == this._elemB) {
			this._objB.setPosition(this._points[3].getX()
					- this._objB.getWidth() / 2, this._points[3].getY())
		} else {
			this._objB.setPosition(this._points[1].getX()
					- this._objB.getWidth() / 2, this._points[1].getY())
		}
	}
};
...
Message.prototype._updateComponents = function() {
	...
	if (this._name) {
		if (this._elemA == this._elemB) {
			...
			this._name.setCoordinates(a - this._name.getWidth() / 2, g
					- this._name.getHeight())
		}
	}
	if (this._stereotype) {
		if (this._name) {
			if (this._elemA == this._elemB) {
				this._stereotype.setCoordinates(e, g
						- this._stereotype.getHeight() - this._name.getHeight()
						/ 2)
			} else {
				this._stereotype.setCoordinates(a - this._stereotype.getWidth()
						/ 2, g - this._stereotype.getHeight()
						- this._name.getHeight())
			}
		} else {
			if (this._elemA == this._elemB) {
				this._stereotype.setCoordinates(e, g
						- (this._stereotype.getHeight()) / 2)
			} else {
				this._stereotype.setCoordinates(a - this._stereotype.getWidth()
						/ 2, g - this._stereotype.getHeight())
			}
...
CreateMessage.prototype.remove = function() {
	...
	for (var b = 0; b < a.length; b++) {
		if (a[b]._elemA._y > a[b]._elemB._y || a[b] instanceof CreateMessage) {
			a[b].setLimitY(a[b]._elemA.getY()
					+ a[b]._elemA._heightSmallRectangle + 5)
		} else {
			a[b].setLimitY(a[b]._elemB.getY()
					+ a[b]._elemB._heightSmallRectangle + 5)
		}
	}
};
...
CreateMessage.prototype._calculateLineEnds = function(l) {
	...
	this._points[0].setPoint(this._elemA
			.getLineX((this._elemA._x > this._elemB._x) ? -1 : 1), this._y);
	if (this._elemB.getX() > this._elemA.getX()) {
		this._points[1].setPoint(this._elemB.getX(), this._y)
	} else {
		this._points[1].setPoint(this._elemB.getX() + this._elemB.getWidth(),
				this._y)
	}
	...
	if (l) {
		...
		for (d = 0; d < n.length; d++) {
			if (!n[d]._moved) {
				...
				n[d].setLimitY(this._y + this._elemB._heightSmallRectangle / 2
						+ 5);
		...
		if (this._elemB.getCreate() != 0) {
			for (d = 0; d < n.length; d++) {
				if (n[d]._y < this._elemB.getCreate()
						&& !(n[d]._elemA instanceof TimeInterval)
						&& !(n[d]._elemB instanceof TimeInterval)) {
					var m = n[d]._y;
					n[d]._points[0].setY(m + this._elemB.getCreate()
							- this._elemA.getY());
					n[d]._points[1].setY(m + this._elemB.getCreate()
							- this._elemA.getY());
					n[d]._y = m + this._elemB.getCreate() - this._elemA.getY();
					n[d].setLimitY(this._y + this._elemB._heightSmallRectangle
							/ 2 + 5);
					if (n[d]._limitY > n[d]._y) {
						n[d]._y = n[d]._limitY
					}
...
CreateMessage.prototype.descendantsCreateMessages = function() {
	...
	var e = false;
	for (d = 0; d < a.length; d++) {
		if ((a[d] instanceof CreateMessage) && (a[d]._elemA == this._elemB)
				&& (a[d]._elemA != a[d]._elemB)) {
			e = false;
	...
	if (this._selectedPoint
		...
		if (b) {
			if (this._selected == 0) {
				if (b != this._elemB
						&& b != this._objA
						&& ((b.getType() == "UMLLifeline" && !b.getDelete()) || b
								.getType() == "TimeInterval")) {
					this.setElementA(b)
				}
			} else {
				if (b != this._elemA && b.getType() == "UMLLifeline"
						&& !b.getDelete()) {
					this.setElementB(b)
				}
			}
...
Lifeline.prototype.isOver = function(a, b) {
	...
	if ((a >= this._x && a <= this._x + this._width && b >= this._y && b <= this._y
			+ this._heightSmallRectangle)
			|| (a >= (this.getLineX() - 5) && a <= (this.getLineX() + 5)
					&& b >= (this._y + this._heightSmallRectangle) && b <= (this._y + this._height))) {
		return true
	}
	return false
};
Lifeline.prototype.notifyChange = function() {
	...
	this.setHeightSmallRectangle(this._components[0].getHeight()
			+ this._components[1].getHeight());
	var c = this._heightSmallRectangle - d;
	for (b = 0; b < this._relations.length; b++) {
		if (!(this._relations[b] instanceof CreateMessage)
				|| ((this._relations[b] instanceof CreateMessage) && (this._relations[b]._elemA == this))) {
			if (c) {
				if (!(this._relations[b] instanceof CreateMessage)) {
					if (this._relations[b]._elemA == this) {
						if ((this._relations[b]._elemB._y
								+ this._relations[b]._elemB._heightSmallRectangle + 5) < (this._y
								+ this._heightSmallRectangle + 5)) {
							this._relations[b].setLimitY(this._y
									+ this._heightSmallRectangle + 5)
						} else {
							this._relations[b]
									.setLimitY(this._relations[b]._elemB._y
											+ this._relations[b]._elemB._heightSmallRectangle
											+ 5)
						}
					} else {
						if ((this._relations[b]._elemA._y
								+ this._relations[b]._elemA._heightSmallRectangle + 5) < (this._y
								+ this._heightSmallRectangle + 5)) {
							this._relations[b].setLimitY(this._y
									+ this._heightSmallRectangle + 5)
						} else {
							this._relations[b]
									.setLimitY(this._relations[b]._elemA._y
											+ this._relations[b]._elemA._heightSmallRectangle
											+ 5)
						}
					}
				} else {
		...
		if (c && this._relations[b] instanceof CreateMessage
				&& (this._relations[b]._elemB == this)) {
			this._relations[b].notifyChange()
		}
		if (c) {
			this.resetMovement()
...
Lifeline.prototype.updateDelete = function() {
	...
	for (h = 0; h < p.length; h++) {
		if (p[h] instanceof Message) {
			if (!(p[h] instanceof DeleteMessage)
					|| (p[h] instanceof DeleteMessage && p[h]._elemB != this)) {
				g = p[h].getY();
...
Lifeline.prototype.updateLength = function() {
	...
	if (this._delete) {
		for (e in this._relations) {
			if ((this._relations[e] instanceof Message)
					&& this._relations[e].getY() > h) {
...
Lifeline.prototype.drawShape = function(a) {
	...
	a.strokeRect(JSGraphic.toPixel(this._x), JSGraphic.toPixel(this._y),
			this._width, this._heightSmallRectangle);
	a.beginPath();
	a.moveTo(this.getLineX(), JSGraphic.toPixel(this._y)
			+ this._heightSmallRectangle);
...
var UMLSequenceDiagram = function(b) {
	...
	a
			.setValidElements([ "UMLNote", "UMLLine", "UMLCreate",
					"UMLDestroy", "UMLLifeline", "UMLCallMessage",
					"UMLSendMessage", "UMLDeleteMessage", "UMLReplyMessage",
					"UMLOption", "UMLAlternative", "UMLLoop", "UMLBreak",
					"Region", "TimeInterval" ]);
	return a
};
...
CompositeState.prototype.updateContainer = function(e) {
	...
	if (this._container) {
		...
		if (g < this._x || d < (this._y + this._heightSmallRectangle)) {
			this.setWidth(this._x - g + this._width);
			this.setHeight(this._y + this._heightSmallRectangle - d
					+ this._height);
			this._x = g;
...
CompositeState.prototype.setElementXML = function(d) {
	...
	this._includeComponentByRegion = (this._includeComponentByRegion == "true") ? true
			: false;
	var b;
	var c = d.childNodes;
	for (b = 0; b < c.length; b++) {
		if (c[b].nodeName == "item") {
			if (c[b].getAttribute("behaviors") != null
					&& (c[b].getAttribute("visible") == "true" || c[b]
							.getAttribute("visible") == "false")) {
				this.setValue(c[b].getAttribute("id"), c[b]
						.getAttribute("value"), c[b].getAttribute("behaviors"),
						c[b].getAttribute("visible"))
			} else {
				this.setValue(c[b].getAttribute("id"), c[b]
						.getAttribute("value"))
			}
...
CompositeState.prototype.getLinkCentered = function(o, n) {
	...
	if (o < d) {
		if (n < c) {
			b = (this.getY() + this._heightSmallRectangle - c)
					/ (this.getX() - d);
			if (((n - c) == 0) || Math.abs((n - c) / (o - d)) < Math.abs(b)) {
				...
				if (Math.abs((n - c) / (o - d)) < Math.abs(b)) {
					...
					if (Math.abs((n - c) / (o - d)) < Math.abs(b)) {
						...
						var e = this.getX() + this._Xmovement
								+ this._widthSmallRectangle;
						b = (this.getY() - c) / (e - d);
						if (Math.abs((n - c) / (o - d)) < Math.abs(b)) {
							a = this.getX();
							p = this.getY();
		...
		if (n < c) {
			...
			if (((n - c) == 0) || ((n - c) / (o - d)) < b) {
				b = (this.getY() - c)
						/ (this.getX() + this._Xmovement
								+ this._widthSmallRectangle - d);
				if ((e > (this.getX() + this.getWidth() / 2))
						&& Math.abs((n - c) / (o - d)) > Math.abs(b)) {
					h = this.getX() + this.getWidth();
					g = this.getY()
				} else {
					b = (this.getY() + this._heightSmallRectangle - c)
...
CompositeState.prototype.setVisibility = function(a) {
	...
	for (var b = 0; b < this._components.length; b++) {
		if ((c || (!c && this._components[b]._id == "name"))
				&& (!(this._components[b] instanceof SpecificationItem) || this._components[b] instanceof SpecificationItem
						&& this._components[b].getValue() != "")) {
...
CompositeState.prototype.isOver = function(a, b) {
	...
	if ((a >= this._x + this._Xmovement
			&& a <= this._x + this._Xmovement + this._widthSmallRectangle
			&& b >= this._y && b <= this._y + this._heightSmallRectangle)
...
CompositeState.prototype.setValue = function(e, c, a, d) {
	var b;
	for (b in this._components) {
		if (!(this._components[b] instanceof SuperComponent)
				&& this._components[b].getId() == e) {
...
CompositeState.prototype.drawFigures = function(b) {
	...
	for (a = 0; a < this._figures.length; a += 1) {
		if (a == this._selectedFigure) {
			this._figures[a].draw(b, this._x, this._y, this._width,
					this._height, this._heightSmallRectangle,
					this._widthSmallRectangle, this._Xmovement)
		}
	}
};
...
SimpleState.prototype.draw = function(a) {
	...
	if (this._components[2]._visible) {
		...
		a.fillStyle = "#000000";
		a.beginPath();
		a.moveTo(JSGraphic.toPixel(this.getX()), JSGraphic.toPixel(this.getY())
				+ this._components[0].getHeight()
				+ this._components[1].getHeight());
		a.lineTo(JSGraphic.toPixel(this.getX() + this.getWidth()), JSGraphic
				.toPixel(this.getY())
				+ this._components[0].getHeight()
				+ this._components[1].getHeight());
		a.closePath();
...
var UMLStateMachineDiagram = function(b) {
	...
	a.setValidElements([ "UMLNote", "UMLLine", "UMLInitialPseudostate",
			"UMLFinalState", "UMLTerminate", "UMLEntryPoint", "UMLExitPoint",
			"UMLJunction", "UMLSimpleState", "UMLCompositeState",
			"UMLVerticalRegion", "UMLHorizontalRegion", "UMLTransition",
			"Region" ]);
	return a
};
...
var UMLCompositeState = function(b) {
	...
	a.setHeightSmallRectangle(a._components[0].getHeight()
			+ a._components[1].getHeight());
...
var UMLUseCaseDiagram = function(b) {
	...
	a.setValidElements([ "UMLNote", "UMLLine", "UMLActor", "UMLUseCase",
			"UMLUseCaseExtended", "UMLSystem", "UMLSubSystem",
			"UMLCommunication", "UMLExtend", "UMLInclude", "UMLGeneralization",
			"UMLGeneralizationSet", "SetLine" ]);
	return a
};
